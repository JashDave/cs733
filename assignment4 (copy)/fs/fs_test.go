...
func TestInit(t *testing.T) {
	fsys = GetFileSystem(1000)
}

func TestFS_BasicSequential(t *testing.T) {
	// Read non-existent file cs733
	m := fsys.ProcessMsg(&Msg{Kind: 'r', Filename: "cs733"})
	expect(t, m, &Msg{Kind: 'F'}, "file not found")

	// Delete non-existent file cs733
	m = fsys.ProcessMsg(&Msg{Kind: 'd', Filename: "cs733"})
	expect(t, m, &Msg{Kind: 'F'}, "file not found")

	// Write file cs733
	str := "Cloud fun"
	m = fsys.ProcessMsg(&Msg{Kind: 'w', Filename: "cs733", Contents: []byte(str)})
	...
	str2 := "Cloud fun 2"
	// Cas new value
	m = fsys.ProcessMsg(&Msg{Kind: 'c', Filename: "cs733", Contents: []byte(str2), Version: version})
	...
	m = fsys.ProcessMsg(&Msg{Kind: 'c', Filename: "cs733", Contents: []byte(str), Version: version})
...
func TestFS_BasicTimer(t *testing.T) {
	...
	str := "Cloud fun"
	m := fsys.ProcessMsg(&Msg{Kind: 'w', Filename: "cs733", Contents: []byte(str), Exptime: 2})
	expect(t, m, &Msg{Kind: 'O'}, "write success")
	...
	m = fsys.ProcessMsg(&Msg{Kind: 'w', Filename: "cs733", Contents: []byte(str), Exptime: 1})
	expect(t, m, &Msg{Kind: 'O'}, "file recreated")

	// Overwrite the file with expiry time of 4. This should be the new time.
	m = fsys.ProcessMsg(&Msg{Kind: 'w', Filename: "cs733", Contents: []byte(str), Exptime: 3})
...
func TestFS_ConcurrentWrites(t *testing.T) {
	...
	for i := 0; i < nclients; i++ {        // 100 clients
		go func(i int) {
			...
			for j := 0; j < niters; j++ {
				...
				m := fsys.ProcessMsg(&Msg{Kind: 'w', Filename: "concWrite", Contents: []byte(str)})
				ch <- m
			}
		}(i)
	}
	...
	m := fsys.ProcessMsg(&Msg{Kind: 'r', Filename: "concWrite"})
...
func TestFS_ConcurrentCas(t *testing.T) {
	...
	nclients := 100
	niters := 10
	m := fsys.ProcessMsg(&Msg{Kind: 'w', Filename: "concCas"})
	ver := m.Version
	if m.Kind != 'O' || ver == 0 {
