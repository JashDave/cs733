...
func TestRPCMain(t *testing.T) {
	DIRNAME := "TestLog"
	ids := []int{100, 200, 300, 400, 500}
	nc := []raft.NetConfig{{uint64(ids[0]), "localhost", uint16(5001)},
	...
	EleTimeout := uint64(5000)
	HBTimeout := uint64(2000)

	for i := 0; i < len(nc); i++ {
		os.RemoveAll(DIRNAME + strconv.Itoa(int(ids[i])))
	}

	for idx:=0;idx<len(nc);idx++ {
		if idx==0 {
			conf := raft.Config{nc, uint64(ids[idx]), DIRNAME + strconv.Itoa(int(ids[idx])), EleTimeout-1000, HBTimeout}
			go StartServer(addr[idx],conf)
		} else {
