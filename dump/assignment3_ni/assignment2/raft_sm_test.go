...
func TestVoteReqToFollowerWithSameTermSameLog(t *testing.T) {
	...
	eventChan <- CreateEvent("VoteReq", "term",sm.currentTerm, "candidateId",uint64(50), "lastLogIndex",sm.logIndex-1, "lastLogTerm",sm.logElementAt(sm.logIndex-1).Term)
	sm.processEvent()
	//Must remain in follower mode
	if sm.state!=FOLLOWER {
		t.Error("Invalid mode\n")
...
//save for later use
	term := sm.currentTerm
	eventChan <- CreateEvent("AppendEntriesReq","term", sm.currentTerm+2, "leaderId", sm.peers[0], "prevLogIndex", uint64(0), "prevLogTerm", sm.logElementAt(0).Term, "entries", LogEntry{0,[]byte{},false}, "leaderCommit", uint64(0))
	sm.processEvent()
	//Must change to follower state
	if sm.state != FOLLOWER {
		t.Error("Invalid mode\n")
...
//save for later use
	term := sm.currentTerm
	eventChan <- CreateEvent("AppendEntriesReq","term", sm.currentTerm+2, "leaderId", sm.peers[0], "prevLogIndex", uint64(0), "prevLogTerm", sm.logElementAt(0).Term, "entries", LogEntry{0,[]byte{},true}, "leaderCommit", uint64(1))
	sm.processEvent()
...
func TestAppendEntriesReqToFollowerWithHigherCommit(t *testing.T) {
	...
	term := sm.currentTerm
	eventChan <- CreateEvent("AppendEntriesReq","term", sm.currentTerm, "leaderId", sm.peers[0], "prevLogIndex", uint64(0), "prevLogTerm", sm.logElementAt(0).Term, "entries", LogEntry{0,[]byte{},true}, "leaderCommit", sm.logIndex+5)
	sm.processEvent()
	//Must change to follower state
	if sm.state != FOLLOWER {
		t.Error("Invalid mode\n")
...
func TestAppendEntriesReqToFollowerWithLargerPrevIndex(t *testing.T) {
	...
	sm.commitIndex = uint64(0)
	term := sm.currentTerm
	eventChan <- CreateEvent("AppendEntriesReq","term", sm.currentTerm, "leaderId", sm.peers[0], "prevLogIndex", uint64(sm.log.GetLastIndex()+6), "prevLogTerm", sm.logElementAt(0).Term, "entries", LogEntry{0,[]byte{},true}, "leaderCommit", sm.logIndex)
	sm.processEvent()
	//Must change to follower state
	if sm.state != FOLLOWER {
		t.Error("Invalid mode\n")
