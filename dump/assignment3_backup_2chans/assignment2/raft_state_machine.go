...
func (sm *StateMachine) AppendEntriesReq(term uint64, leaderId uint64, prevLogIndex uint64, prevLogTerm uint64, entries LogEntry, leaderCommit uint64) {
	...
	sm.actionChan <- CreateAction("Send", "peerId", leaderId, "event", event)
}
...
//fmt.Println("SM CmtIdx:",sm.commitIndex)
				sm.actionChan <- CreateAction("Commit", "index", sm.commitIndex, "data", sm.logElementAt(sm.commitIndex).Data, "err", nil)
			}
			return true
		}
...
//fmt.Println("SM CmtIdx:",sm.commitIndex,"NI",sm.nextIndex)
					sm.actionChan <- CreateAction("Commit", "index", ni-1, "data", sm.logElementAt(ni-1).Data, "err", nil)
				}
			}
