...
func GetFollowerSM() (*StateMachine){
	sm := InitStateMachine(uint64(10), []uint64{20,30,40,50}, uint64(3), time.Duration(500)*time.Millisecond, time.Duration(200)*time.Millisecond, uint64(1), uint64(0), []LogEntry{LogEntry{0,nil,false}})
	return sm
}

func GetCandidateSM() (*StateMachine){
	sm := InitStateMachine(uint64(10), []uint64{20,30,40,50}, uint64(3), time.Duration(500)*time.Millisecond, time.Duration(200)*time.Millisecond, uint64(2), uint64(10), []LogEntry{LogEntry{0,nil,false}})
	sm.state = CANDIDATE
	return sm
}
...
//save for later use
	term := sm.currentTerm
	eventChan <- CreateEvent("AppendEntriesReq","term", sm.currentTerm+2, "leaderId", sm.peers[0], "prevLogIndex", uint64(0), "prevLogTerm", sm.log[0].Term, "entries", LogEntry{0,[]byte{},false}, "leaderCommit", uint64(1))
	sm.processEvent()
	//Must change to follower state
	if sm.state != FOLLOWER {
		t.Error("Invalid mode\n")
