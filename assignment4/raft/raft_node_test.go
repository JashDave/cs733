...
func TestMultipleAppend(t *testing.T) {
	...
	for _, node := range rn {
		//fmt.Println("NODE ID :", node.Id(), "LastLogIndex", node.rnlog.GetLastIndex())
		for i := uint64(1); i <= node.CommittedIndex(); i++ {
			err, data := ldr.Get(i)
			...
			if string(data) != fmt.Sprintf("%d",i) {
				t.Fatal("Incorrect data in log  NODE ID :", node.Id(), " Index :",i)
			}
		}
	}
}
